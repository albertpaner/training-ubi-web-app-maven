...
public class UtenteDao implements Crud<UtenteBean> {
	...
	/**
	 * This method corresponds to READ operation on the database. It returns a list
	 * of all the users in the database.
	 *
	...
	/**
	 * this method corresponds to CREATE operation on the database. It creates a new
	...
	/**
	 * this method corresponds to UPDATE operation on the database. It updates an
	...
	public int update(List<Object> userParams) throws SQLException, ClassNotFoundException {
		...
		int rs = stmt.executeUpdate("UPDATE utente SET email = '" + email + "', password = '" + password
				+ "', ruolo_id = " + ruoloId + ", nome = '" + nome + "', cognome = '" + cognome + "', valutatore_id = "
				+ valutatoreId + ", data_nascita = '" + dataNascita + "', in_sospeso = " + (inSospeso ? 1 : 0)
	...
	/**
	 * this method corresponds to DELETE operation on the database. It logically
	...
	/**
	 * this method corresponds to DELETE operation on the database. It physically
	 * deletes an existing user in the database.
	 *
	 * @param utenteId The ID of the user to be deleted.
	...
	/**
	 * This method updates the last access time of a user in the database. The last
	...
	public int createVal(List<Object> userParams) throws SQLException, ClassNotFoundException {
		...
		String password = (String) userParams.get(1);
		int ruoloId = 1;
		String nome = (String) userParams.get(2);
		String cognome = (String) userParams.get(3);
		Date dataNascita = (Date) userParams.get(4);

		Statement stmt = conn.createStatement();
		int rs = stmt.executeUpdate(
				"INSERT INTO utente (email, password, ruolo_id, nome, cognome, data_nascita) VALUES ('" + email + "', '"
						+ password + "', " + ruoloId + ", '" + nome + "', '" + cognome + ", '" + dataNascita + "')");
		conn.close();
		return rs;
	}

	public boolean createVal(String email, String password, String nome, String cognome, Timestamp dataUltMod,
			Timestamp dataCreaz, Timestamp dataUltAcc) {
	...
	public List<UtenteBean> getUtentiOrdinatiPerEta() throws ClassNotFoundException {
		Connection conn = null;
		PreparedStatement stmt = null;
		ResultSet rs = null;
		List<UtenteBean> utentiOrdinatiPerEta = new ArrayList<>();

		try {
			this.conn = DBConnection.createConnection();
			String query = "SELECT * FROM utenti ORDER BY data_nascita ASC LIMIT 2";
			...
			while (rs.next()) {
				...
				utente.setUtenteId(rs.getInt("id"));
				utente.setNome(rs.getString("nome"));
				utente.setCognome(rs.getString("cognome"));
				utente.setDataNascita(rs.getDate("data_nascita"));
				char primaLettera = utente.getNome().toLowerCase().charAt(0);
                if (isConsonante(primaLettera)) {
                    utente.setInSospeso(true);
                }
			
				utentiOrdinatiPerEta.add(utente);
			}
